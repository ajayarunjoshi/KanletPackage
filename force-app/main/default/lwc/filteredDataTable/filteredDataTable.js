...
import getContacts from '@salesforce/apex/FileredDataTableController.getContacts';
...
handleContactNameChange(event) {
    this.ContactNameValue = event.detail.value;
    this.selectedAccountIds = this.SelectedRecords.map(row => row.Id);
    this.getContactDetails(this.selectedAccountIds);
}
...
async getContactDetails() {
    try {
        const result = await getContacts({ 
            selectedAccounts: this.selectedAccountIds, 
            recordId: this.recordId,
            name: this.ContactNameValue,
            accName: this.AccountValue,
